...
static GGKMessageReceived messageReceivedCallback = NULL;
static char * ggk_ble_brand = NULL;
static char * ggk_ble_model = NULL;
static char * ggk_sender_char = NULL;
static char * ggk_receiver_char = NULL;

static char ggk_sender_cache[500] = {0};
static int  ggk_sender_cache_len = 0;
...
void ggkServerRegisterReceiverCB( const char * ch, GGKMessageReceived receivedCB )
{
    ...
    messageReceivedCallback= receivedCB;
}

void ggkServerSendMessage( const char * message, int size )
{
    memset(ggk_sender_cache,0,500);
    memcpy(ggk_sender_cache,message,size);
    ggk_sender_cache_len = size;

    if(ggk::HciAdapter::getInstance().getActiveConnectionCount()>0){

        ggkNofifyUpdatedCharacteristic("/com/bleggklinux/msg_service/msg_send");
    }
}
	...
	GGKServerDataGetter getter, GGKServerDataSetter setter, const RawAdvertisingData &advData )
{
    ...
    if (advData.advDataLen||advData.rspDataLen)
    {
        this->adv = advData;
        this->useRawAdvertisingData = true;
    }
	...
	enableAdvertising = !this->useRawAdvertisingData;
	...
	.gattServiceBegin("device", "180A")
		...
		.gattCharacteristicBegin("brand", "2A29", {"read"})

			// Standard characteristic "ReadValue" method call
			.onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				self.methodReturnValue(pInvocation, ggk_ble_brand, true);
			})
		...
		.gattCharacteristicBegin("model", "2A24", {"read"})
			...
			.onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				self.methodReturnValue(pInvocation, ggk_ble_model, true);
			})
	...
	.gattServiceBegin("msg_service", "CAD0")


            // FF81特征值(read, write)
            .gattCharacteristicBegin("msg_receive", "6b44", { "write"})

//			// 读操作回调
            .onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
                         {
                             const char *pTextString = self.getDataPointer<const char *>("msg_service/msg_receive", "");
            ...
            .onWriteValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
            {
                ...
                char ble_buffer[500] = {0};
                int ble_buffer_len;

                Utils::BytesFromGVariantByteArray( pAyBuffer, ble_buffer, &ble_buffer_len );

                messageReceivedCallback( ble_buffer, ble_buffer_len );
            ...
            .gattCharacteristicEnd()
                    ...
                    // FF82特征值(indicate)
            .gattCharacteristicBegin("msg_send", "6b64", {"indicate"})
